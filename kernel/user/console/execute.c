...
int console_execute_overwrite(Application *app) {
    ...
    if (!strncmp(cmd, "let ", 4)) {
        ...
        fb_write_ansi(win, "  \033[32mkill\033[0m X   - Kills app or file X. Example: app1, file3\n");
        fb_write_ansi(win, "  \033[32mto\033[0m X V   - Sends message V to app X's input\n");
        fb_write_ansi(win, "  \033[32margs\033[0m     - Retrieves the last message's contents\n");
